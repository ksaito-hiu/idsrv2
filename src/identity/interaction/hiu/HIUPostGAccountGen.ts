...
export class HIUPostGAccountGen implements PostGAccountGen {
  ...
  public constructor(webIdStore: WebIdStore, webIdBase: string, hiuStorage: JsonResourceStorage<HiuData>) {
  ...
  createIdsFromEMail(email: string) {
    if (email.endsWith('@s.do-johodai.ac.jp')) {
      return {
        webId: this.webIdBase+"s20"+email.substring(1,8)+'#me',
...
console.log('GAHA: HIUAuthFilter, email=',email);
      // ちょっと自分だけ前提でテスト
      return {
        webId: this.webIdBase+'f200088071#me',
        hiuId: 'f200088071'
      };
